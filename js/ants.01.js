...
$(function () {
	...
	(function () {
		...
		var antsInterface = {
			...
			// Setter for the antSize
			setAntSize : function (val) {
				this.antSize = val;
			},
			...
			// Calculate real pos on screen by multiplying with antSize
			reMap : function (val) {
				return val * this.antSize;
			},
			...
			drawPixel : function (pos) {
				...
				ctx.fillRect(this.reMap(pos[0]), this.reMap(pos[1]), this.antSize, this.antSize);
		...
		var antController = {
			...
			init : function () {
				...
				antsInterface.setAntSize($('input.size').val());
				antsInterface.pixels = [];
				antBrain.setBoundaries(0, antsInterface.dimensions.width, antsInterface.dimensions.height, 0, antsInterface.antSize);
				antBrain.resetStepCounter();
